...
function connectDb(alias, path) {
  var defer = Q.defer();
  console.log(path, 'MONGOOSEPATH');
  var connection = mongoose.createConnection (path).then(function() {
     console.log(arguments, 'are im now connected?');
      connectDbOk.bind(null, defer, {path:path, alias: alias, connection: connection})
  }, function(err){
      connectDbFailed.bind(null, defer, {});
  });
    ...
    //console.log(database, 'or', db);
      database.then(function(dbcon) {
        console.log('dbcon', 'err or db');
          ...
          Q.allSettled (db_promises).then(dataBasesReady.bind(null, defer, dbcon));
      }.bind(this));
