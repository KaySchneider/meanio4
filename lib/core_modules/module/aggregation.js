...
Asset.prototype.storeJS = function(){
  ...
  if(process.env.NODE_ENV == 'development') {
      /**
       * add jsx files for development values inside the mean.io stack
       */
      if(typeof this.options !== 'undefined' && typeof this.options.jsx !== 'undefined') {
          console.log('develjsx');
              aggregated.js['develjsx'].add(this);
...
function onAggregatedSrc(loc,ext,res,next,data){
    /**if(loc=='develjsx')
        console.log(res.locals.aggregatedassets[loc][ext], 'DATABUNDLE', loc,ext);
    else**/
    res.locals.aggregatedassets[loc][ext] = data;
    //console.log('try to add more');
    next && next();
}

function aggregationRequestHandler(meanioinstance, req, res, next){
  //res.locals.assets = assets;
  res.locals.aggregatedassets = {header:{},footer:{}, develjsx:{}, };
  ...
  meanioinstance.aggregatedsrc('jsx', 'develjsx', onAggregatedSrc.bind(null,'develjsx','js',res,next));
...
function supportAggregate(Meanio) {
  ...
  Meanio.prototype.aggregated = function(ext, group, callback) {
    ...
    //this will throw an error if group is no object attribute of aggregated.js
