/**
 * patched only the line 63 to 74
 * the original is inside the comment. Mean Core Version 4.4
 * @type {exports}
 */
...
function ExpressEngine(){
    console.log('START EXPRESS', this)
...
ExpressEngine.prototype.initApp = function(){
    ...
    this.app.use(function(req,res,next){
        res.setHeader('X-Powered-By','YAKISCH ENGINE [yakisch.com] / EMPPLAN');
        next();
    });
    ...
    /**
     * TODO: the mean.io people set here the init for json and urlendoced without any limit
     * so it falls back to the limit.
     * When you try to override this inside the expresss settings. There is every request an instance
     * with an small limit.
     * Because, we add than an new Middleware with the bodyParser
     *
    ...
    if(typeof(config.bodyParser) !== 'undefined') {
        var bconfig = config.bodyParser;
        this.app.use(bodyParser.json(bconfig.json));
        this.app.use(bodyParser.urlencoded(bconfig.urlencoded));
        this.app.use(bodyParser.raw(bconfig.raw));
    } else {
        //TODO:clear the code
        this.app.use(bodyParser.json());
        this.app.use(bodyParser.urlencoded({
    ...
    try {
        //console.log('EXPRESS MONGO SESSION STORAGE!',this.db.connection.db);
        ...
        console.log('broken part!', e);
    } finally  {
        console.log('STARTUP EXPRESS STUFF')
    }
...
function NotFoundHandler(err, req, res, next) {
    ...
    // Error page 500 show only in development
